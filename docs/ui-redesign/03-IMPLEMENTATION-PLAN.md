...
- [x] E5. **Quick Access spotlight** — global `Ctrl/Cmd+Shift+Space` (1Password-style). New main-process `QuickAccessMain` (`apps/desktop/src/main/quick-access.main.ts`) registers the global shortcut via Electron `globalShortcut`, restores/focuses the window, and broadcasts `"openQuickAccess"`; the redesign shell subscribes via `BroadcasterService` and opens the command palette. No local `HostListener` for the Space combo — `globalShortcut` fires even while the window is focused (a local binding would double-fire). The in-app palette keybind stays `Ctrl/Cmd+K`. Wired into `main.ts` (additive). **Renderer + main builds both pass; end-to-end behavior needs a full `npm run electron` run to verify the OS-level shortcut + window summon.**
