...
@Component({
  ...
  template: `
    ...
    >
      <!-- Search input -->
      <div
        class="tw-flex tw-items-center tw-gap-3 tw-px-5 tw-py-4"
      ...
      <div class="tw-max-h-[320px] tw-overflow-y-auto tw-p-2">
        @for (section of sections(); track section.title) {
          <div
            class="tw-px-3 tw-pb-1 tw-pt-3 tw-text-[11px] tw-font-semibold tw-uppercase tw-tracking-[0.08em] tw-text-fg-body-subtle"
          >
            {{ section.title }}
          </div>
          @for (action of section.actions; track action.id; let i = $index) {
            <button
              ...
              [style.background-color]="
                isHighlighted(action.id) ? 'var(--fk-selected-bg)' : 'transparent'
              "
              [style.border]="
                isHighlighted(action.id) ? 'var(--fk-glass-border)' : '1px solid transparent'
              "
            ...
            >
              <span
                ...
                [style.background-color]="
                  isHighlighted(action.id) ? 'var(--fk-card-bg)' : 'transparent'
                "
              ...
              @if (action.shortcut) {
                <kbd class="tw-text-[11px] tw-font-medium tw-text-fg-body-subtle">{{
                  action.shortcut
                }}</kbd>
              }
            </button>
